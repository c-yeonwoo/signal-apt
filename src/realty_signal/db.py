...
def _migrate(c: sqlite3.Connection) -> None:
    if _migrated[0]:
        return
    _migrated[0] = True
    # koczip 제거(2026-09-06) — 운영자가 저장·재배포를 명시적으로 금지했으므로
    # 남은 테이블도 지운다. 수집이 403 으로 막히면서 이미 0건이었다.
    c.executescript("DROP TABLE IF EXISTS koczip_article;"
                    "DROP TABLE IF EXISTS koczip_complex;")
    d = DB.parent
    # geocode.db → geocode
    old = d / "geocode.db"
