...
- 콕집 매물: **2026-09-06 전면 중단.** koczip.com 이 전 지역 요청을 403 으로 차단하면서
  "무단 크롤링·스크래핑·복제·저장·재배포 금지"를 명시적으로 고지했다. 기술적 차단과 권리 유보가
  동시에 있었으므로 **재시도·우회 대상이 아니다.**
  - `ingest/koczip.py` `DISABLED=True` — `_get()` 단일 관문에서 막아 **네트워크 호출이 나가지 않는다.**
    브라우저 위장 헤더(`_HDR`: User-Agent·Origin·Referer)는 제거했다.
  - 자동 수집(부팅 시딩·주간 루프) 호출부 제거. `/api/koczip/*` 는 전부 **410**,
    `meta.allowed=False` 로 화면 탭·버튼이 숨는다.
  - **수집만 막고 저장분을 계속 보여주면 재배포가 이어진다** → 조회도 함께 닫았다.
  - 재개 조건: koczip 과 **서면 이용 허락**. 그 전에는 켜지 않는다.
  - 남은 저장 데이터(`koczip_complex`·`koczip_article`)는 참조되지 않는다. 삭제는 운영자 판단.
