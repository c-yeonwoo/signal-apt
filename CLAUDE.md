...
> **등급식의 정본은 `signals/engine.py:_classify` 하나다.** 문서와 어긋나면 코드가 맞다.
> 2026-08-17 진단 전까지 이 섹션이 **정반대**로 적혀 있었고, 그걸 믿고 만든
> `brain/calibrate.py` 가 **등급식에 없는 파라미터를 스윕**하고 있었다. 문서 오류가 검증 도구를 오염시킨 사례다.

- **매수우위지수**(`buyer_superiority` = 100+매수세우위-매도세우위): **실제 트리거.**
  `≥ buyeridx_strong(70)` 이면 강세 1표. 등급 = `sum([전세난, 매수우위지수≥70, 매매상승])`.
- **매수세우위**(raw, `buyer_demand`): 원본 메모의 "5/10/15/20, 20↑ 매수" 사다리.
  **등급식에 쓰이지 않는다** — `≥20` 이면 `[참고]` 근거 문자열만 붙는다.
  `SignalConfig.demand_*` 는 남아 있지만 소비처가 근거 문구뿐이다.
- 사다리를 등급에 되살릴지는 미결(`docs/audit-2026-08-17.md` N8, V-1·V-2 선행).
...
홈은 **2열**이다(`index.html:1683-1701`). 세로 3장이 아니다 — 2026-08-17 진단 전까지 이 문서가
"상단 3장"이라고 적어 뒀는데 코드와 달랐다:

| 열 | 라벨 | 카드 순서 |
|---|---|---|
| 좌 | `할 일 · 후보` | **매수력**(시각 정점, 32px) → 다음 할 일 → 이번 주 볼 단지 → 관심 동네 → 예산 안 BUY+ → 피드 |
| 우 | `주간 동향` | **이번 주 변화** → 경기 국면 → 이번 주 이슈 → 바로가기 |

`@media(max-width:760px)` 에서 1열로 접히면 좌열이 먼저라 **'이번 주 변화'가 7번째 카드**가 된다.
즉 above-the-fold 는 V1(타이밍)이 아니라 V2(매수력)다. 이 순서가 맞는지는 미결
(`docs/audit-2026-08-17.md` N7, V-2 선행). **임의로 바꾸지 말 것.**
...
- 콕집 매물(개인 확인용): `ingest/koczip.py` + SQLite `koczip_complex`/`koczip_article`
  ...
  **수집 주기는 바로집과 다르다**(2026-08-17 변경) — 제3자 사이트를 긁는 개인 확인용 소스라
  배포 서버가 매일 자동으로 붙지 않는다. 세 경로를 구분한다:
  ① **부팅 시딩 = 로컬만**(`is_prod()` 이면 안 함 — 재배포마다 붙는 걸 막는다)
  ② **자동 갱신 = 주 1회**(`_KOCZIP_AUTO_MAX_AGE=7일`, `_koczip_auto_due()`).
     **비어 있으면 자동으로 채우지 않는다** — 첫 수집은 사람이 시작한다
  ③ **수동 = admin 언제든**(`POST /api/koczip/scan`). prod 에서도 막히지 않는다
  `_koczip_stale()`(1일, 화면 표시·수동 판정)과 `_koczip_auto_due()`(7일, 자동 수집)는 **다른 함수**다.
  화면은 "마지막 수집 N일 전"을 항상 표시한다(`_renderKoczipAge`) — 오래된 데이터를 최신처럼 보이면 안 된다.
