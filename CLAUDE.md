...
- 계약금 비율·잔금 단계별 현금흐름·예비비(부대비용×10%)는 미모델링(단계별 표시는 미구현).
...
- 콕집(koczip) 연동은 **2026-09-06 완전 제거**했다. 운영자가 전 지역 요청을 403 으로 차단하며
  "무단 크롤링·스크래핑·복제·저장·재배포 금지"를 명시했다. 기술적 차단과 권리 유보가 동시에 있었으므로
  재시도·우회 대상이 아니었다. 코드·라우트·테이블·화면을 전부 걷어냈고 `db._migrate` 가 남은 테이블을 DROP 한다.
  **되살리지 말 것** — 재개하려면 서면 이용 허락이 먼저다.
  급매·찐매물은 바로집(`ingest/baroezip.py`)만 쓴다. 실측(2026-09-06) 4개 지역 급매 265건·찐매물 125건 정상.
