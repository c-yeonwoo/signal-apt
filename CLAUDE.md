...
- 매수력 확정서(`buying_power.py`): 최대매수가·대출·월상환·필요현금을 한 산출물로. LTV 상한(주택수·생애최초·규제),
 기대출 차감 + 스트레스 DSR, 주택수별 취득세, 중개보수 구간요율, 소득 30% 기준 '안정매수가'.
 `/api/buying-power`(계산) · `/confirm`(프로필 `매수력`에 확정 저장). 확정 가정은 다음 조회의 기본값이 된다.
