...
- 급매 할인율 뷰(콕집): `ingest/koczip.py` → `/api/koczip/quick-deals` (kv 12h 캐시).
  **개인 확인용만** — 로컬 또는 `ADMIN_EMAILS`. 재배포·공개 SaaS 프록시 금지.
  원천은 네이버 호가×국토부 실거래(콕집 집계). 공개 제품 전환 시 이 경로 폐기·자체/제휴로 교체.
  바로집 레이더(급매·찐매물 핀)와 별축 — 콕집은 전국 단지×면적 할인율 테이블.
