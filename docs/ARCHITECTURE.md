...
> 최종 갱신: 2026-07 · **Timing v1** · **Brain layer Phase 0–1**
...
```
┌─────────────────────────────────────────────────────────────────┐
│ L4 Brain     Nick · events · outcome · calibration (진행 중)      │
├─────────────────────────────────────────────────────────────────┤
...
└─────────────────────────────────────────────────────────────────┘
         FastAPI (api.py)  ←→  SPA (web/index.html)
```
...
| **L4 Brain** | 설명·개인화·학습 연료 | `advisor.py`, `events`, `ai_report.py` |
...
| 필드 | 의미 | 시그널 트리거 |
|------|------|---------------|
| `buyer_demand` | 매수세우위 (raw) | 사다리 5/10/15/20 |
| `buyer_superiority` | 매수우위지수 (100=중립) | 차트·참고 |
| `jeonse_supply` | 전세수급지수 | 밴드 100/140/170/190 |
| `sale_change` / `jeonse_change` | 주간 증감% | 4주 모멘텀 |
...
임계값은 `SignalConfig` dataclass에만 정의 (하드코딩 금지).  
버전 관리·자동 보정(Phase 2) 대상.
...
| Rule | 트리거 |
|------|--------|
| `signal_upgrade` | ★ 지역 시그널 **승급** (전역 `signal_changes` 필터) |
...
KB 갱신 시 지역 feature append → Phase 2에서 N주 후 라벨링·보정.
...
**Phase 2+:** `CalibrationProposal`, listing outcome, ML 랭킹 보조.
...
- **단일 앱:** `api.py` (~2700줄) — router 분리는 Phase 0 TODO
- **인증:** 쿠키 `rsm_session`, `/api/*` 게이트
- **캐시:** `@lru_cache` on `_kb()`, `_signals_df()`, `_regime()`
...
_Engine v1은 규칙 기반 Decision + Outcome 적재까지 포함한다. ML·자동 임계값 적용은 Phase 2 이후이며, production 적용 전 수동 검증·백테스트 PR을 원칙으로 한다._
