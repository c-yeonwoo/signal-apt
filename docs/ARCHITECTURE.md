...
└─────────────────────────────────────────────────────────────────┘
         FastAPI (api.py + routes/* + services/market_data)  ←→  SPA (web/index.html)
```
...
| **Complex** | `services.complex_signal` | 사이클·지역·단지·가격 가중 (백테스트 전) |
...
- **앱:** `api.py` 오케스트레이터(헬퍼·lifespan) + 도메인 router — `/api/*` 등록은 router만
  - `routes/auth.py` · `alerts.py` · `advisor.py` · `market.py`
  - `routes/auction.py` · `personal.py`
...
| **5** | Alert Accountability — 알림 사후채점 · 명시적 피드백 | ✅ |

### Phase 5 요약

| 모듈 | 역할 |
|------|------|
| `brain/alert_track.py` | `signal_changes` → 12주 KB 가격지수 hit/miss/pending |
| `GET /api/alerts/track-record` | 로그인 유저 요약(recent≤5) |
| `GET /api/brain/alerts/track-record` | 관리자 상세 + `feedback_summary` |
| `POST /api/alerts/feedback` | 👍/👎 → `events.alert_feedback` |
| 알림 UI | 적중률 한 줄 + 행별 피드백(중복 클릭 방지) |

자동 캘리브레이션 반영·ML 없음. 피드백은 수집·표시만(랭킹 가점 연결은 Phase 6 후보).
...
```
src/realty_signal/
├── api.py                 # FastAPI 오케스트레이터 (도메인 핸들러·헬퍼)
...
│   ├── alert_track.py     # Alert Accountability (Phase 5)
...
├── routes/
│   ├── deps.py
│   ├── auth · alerts · advisor · market · auction · personal
│   ├── complex · redev · strategy · geo
│   └── brain.py
├── signals/
...
_Engine v1 = 규칙 기반 Decision + Outcome 라벨링 + CalibrationProposal(수동 apply) + Alert Track Record(Phase 5).  
임계값·피드백의 자동 production 적용은 하지 않는다 — 제안·집계 후 수동 승인._
