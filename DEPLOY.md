...
3. **환경변수 설정** (Variables 탭): 아래 중 사용하는 것만.
   ...
   ADMIN_EMAILS=a@b.com       # 쓰기 API(경매·KB refresh 등) 관리자
   INVITE_CODES=cohort2026a   # 수강생 초대 코드(콤마 구분) — prod 필수
   STUDENT_ALLOWLIST=         # (선택) 이메일 화이트리스트 — 코드 없이 가입
   # SIGNUP_OPEN=1            # (비권장) cohort 게이트 우회
   APP_BASE_URL=https://signal-apt.up.railway.app
   SMTP_HOST=...              # 주간 digest·비번 재설정 메일
   ODSAY_KEY=...              # (선택)
   ```
   - **가입 게이트**: prod에서 `INVITE_CODES`/`STUDENT_ALLOWLIST` 없으면 신규 가입이 막힙니다. 코호트마다 코드 교체.
